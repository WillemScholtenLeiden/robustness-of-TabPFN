"""
Plotting functions for Experiment 3: Per-Dataset Comparative Evaluation.

Usage:
    data = load_experiment_data("comparison_with_nns")
    plot_experiment_3(data, save_dir="figures")
"""
...
def plot_dataset_comparison(data: dict[str, Any], dataset_idx: int,
    ...
    for model_name in model_names:
        ...
        ax.plot(epsilons, asr_values, f"{MODEL_MARKERS[model_name]}-",
                color=MODEL_COLORS[model_name], label=DISPLAY_NAMES[model_name])
...
def plot_all_datasets_fgsm(data: dict[str, Any], save_dir: str = "figures"):
    ...
    for dataset_idx in range(n_datasets):
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(save_dir, f"exp3_dataset_{dataset_idx + 1}_fgsm.pdf"))
        plt.close(fig)
...
def plot_all_datasets_pgd(data: dict[str, Any], save_dir: str = "figures"):
    ...
    for dataset_idx in range(n_datasets):
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(save_dir, f"exp3_dataset_{dataset_idx + 1}_pgd.pdf"))
        plt.close(fig)
...
def plot_grid_fgsm_pgd(data: dict[str, Any], save_dir: str = "figures"):
    ...
    fig.savefig(os.path.join(save_dir, "exp3_grid.pdf"))
    plt.close(fig)
...
def plot_experiment_3(data: dict[str, Any], save_dir: str = "figures"):
    """Create and save all Experiment 3 figures."""
    _apply_style()
    os.makedirs(save_dir, exist_ok=True)
    ...
    print(f"All Experiment 3 figures saved to '{save_dir}/'.")
